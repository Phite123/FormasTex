...
% Dato: $\QQ(\sqrt{n}) = \QQ(\sqrt{m})$ sii $n=m$ (para libres de cuadrados).
% Demo: por contrarreciproco, si $n \neq m$ y los cuerpos son idem entonces $\sqrt{n} = a+b\sqrt{m}$ y tenes que $a\neq 0$ pq sino $\sqrt{n} = b\sqrt{m} = \sqrt{b^2m}$ no es libre de cuadrados y $b\neq 0$ pq sino $\sqrt{n} = a$ te queda $n=a^2$ no libre de cuadrados. Con eso elevas al cuadrado $n = a^2+b^2m+ab\sqrt{m}$ es racional = racional + irracional absurdo.
