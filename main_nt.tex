...
Al usar la teoria ramificacion de primos en extensiones junto con Galois probamos que hay infinitos primos que se parten completamente y, volviendo al lenguaje de nuestro polinomio $f$, esto nos dice que $\bar{f}\in \FF_p[X]$ se factoriza como producto de lineales, es decir tiene todas sus raices en el cuerpo.
...
% d par, negativo, libre de cuadrados. h_d no mult de 3 
% Vamos a escribir todo lo mismo para $d>0$ pero con el $-1$ correspondiente.

Reescribamos en $\QQ(\sqrt{-d})$ la ecuacion $y^2=x^3-d$ como $(y-\sqrt{-d})(y+\sqrt{-d})=x^3$, y ahora lo que estamos buscando es probar que solo puede haber a lo sumo un $y\in \NN$ tal que $(y-\sqrt{-d})(y+\sqrt{-d})$ es un cubo. 
